...
ORIGIN_DIRECTORY = "/path/to/folder/AtrialFibrillation/"
OUTPUT_DIRECTORY = "/path/to/folder/AtrialFibrillation/"
SPLIT_TRAIN = "train"
SPLIT_TEST = "test"

def process_atrial_fibrillation_data(origin_dir, output_dir, split_train, split_test):
    X_train, y_train = load_from_tsfile(os.path.join(origin_dir, "AtrialFibrillation_TRAIN.ts"))
    X_test, y_test = load_from_tsfile(os.path.join(origin_dir, "AtrialFibrillation_TEST.ts"))

    X_train_numpy = convert_to(X_train, "numpy3D")
    X_test_numpy = convert_to(X_test, "numpy3D")

    # label_map = {"longblink": 0.0, "shortblink": 1.0}
    label_map = {"n": 0.0, "s": 1.0, "t": 2.0}
...
if __name__ == "__main__":
    process_atrial_fibrillation_data(
