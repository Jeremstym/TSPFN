...
class MiniRocketExtractor(nn.Module):
    def __init__(self, num_features=10000):
        super().__init__()
        self.num_features = 84 * (num_features // 84)
        self.parameters = None
        self.is_fitted = False

    def fit(self, X):
        """
        X: np.ndarray of shape (N, Channels, Time)
        """
        print(f"Fitting MiniRocket with {self.num_features} features...")
        # Using the fit function you provided
        self.parameters = fit(X, num_features=self.num_features)
        self.is_fitted = True

    def forward(self, x):
        """
        x: torch.Tensor of shape (N, Channels, Time)
        """
        if not self.is_fitted:
            raise RuntimeError("MiniRocket must be fitted on training data before forward pass.")
        
        # MiniRocket transform is CPU-bound and uses Numba (NumPy)
        # We move to CPU, transform, and move back to the original device
        device = x.device
        x_np = x.detach().cpu().numpy().astype(np.float32)
        
        # Using the Numba transform function you provided
        features = transform(x_np, self.parameters)
        
        return torch.from_numpy(features).to(device)

class MiniRocketClassifier(nn.Module):
    def __init__(self, num_classes, num_features=10000):
        super().__init__()
        self.extractor = MiniRocketExtractor(num_features)
        # The actual trainable part
        self.classifier = nn.Linear(self.extractor.num_features, num_classes)
        
    def fit_extractor(self, X_train):
